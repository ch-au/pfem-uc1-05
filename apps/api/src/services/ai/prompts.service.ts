...
function normalizeQuestion(raw: any): {
  ...
  if (!raw || typeof raw !== "object") {
    throw new Error(
      `Invalid question object: expected object, got ${typeof raw}`,
    );
  }
  ...
  if (!questionText) {
    console.error(
      "   ⚠️ Missing questionText in question. Raw:",
  ...
  if (!correctAnswer) {
    console.error(
      "   ⚠️ Missing correct_answer in question. Raw:",
      JSON.stringify(raw, null, 2),
    );
...
function normalizeQuestionGeneratorOutput(raw: any): QuestionGeneratorOutput {
  ...
  if (rejected === true) {
    console.log(
      `   🚫 Quiz topic rejected by LLM: ${rejectionReason ?? "No reason provided"}`,
    );
    return {
      rejected: true,
      rejection_reason:
        rejectionReason ?? "Dieses Thema kann nicht verarbeitet werden.",
      questions: [],
    };
  }

  // Field name aliases for questions array
  const questionsAliases = [
    "questions",
    "Fragen",
    "fragen",
    "questionList",
  ...
  if (!questionsRaw || !Array.isArray(questionsRaw)) {
    console.error(
      "   ⚠️ Missing or invalid questions array in LLM response. Raw:",
      JSON.stringify(raw, null, 2),
    );
...
function normalizeAnswerGeneratorOutput(raw: any): AnswerGeneratorOutput {
  if (!raw || typeof raw !== "object") {
    throw new Error(
      `Invalid answer generator response: expected object, got ${typeof raw}`,
    );
  }
  ...
  if (!incorrectAnswers && findValue(raw, optionsAliases)) {
    ...
    if (Array.isArray(options) && correctAnswer) {
      ...
      incorrectAnswers = options.filter(
        (opt: string) =>
          opt !== correctAnswer &&
  ...
  if (
    ...
    console.error(
      "   ⚠️ Missing or invalid incorrectAnswers in LLM response. Raw data:",
      JSON.stringify(raw, null, 2),
    );
  ...
  if (incorrectAnswers.length < 3) {
    console.warn(
      `   ⚠️ Only ${incorrectAnswers.length} incorrect answers provided, expected 3`,
    );
  }
  if (incorrectAnswers.length > 3) {
...
export class PromptsService {
  ...
  private async loadPromptTemplate(promptKey: string): Promise<{
    ...
    if (defaults.langfuse_enabled && langfuseService.isActive()) {
      try {
        ...
        if (promptData && promptData.prompt) {
          console.log(
            `✅ Loaded prompt "${promptKey}" from Langfuse (${config.langfuse_name})`,
          );

          // Debug: Log raw Langfuse prompt structure
          console.log(
            `   📋 Langfuse prompt type: ${typeof promptData.prompt}`,
          );
          if (typeof promptData.prompt === "string") {
            console.log(
              `   📋 Langfuse prompt (first 500 chars): ${promptData.prompt.substring(0, 500)}`,
            );
          } else if (Array.isArray(promptData.prompt)) {
            console.log(
              `   📋 Langfuse prompt is CHAT ARRAY with ${promptData.prompt.length} messages`,
            );
            promptData.prompt.forEach((msg: any, i: number) => {
              console.log(
                `      Message ${i}: role=${msg.role}, content (first 200)=${String(msg.content).substring(0, 200)}`,
              );
            });
          } else {
            console.log(
              `   📋 Langfuse prompt object:`,
              JSON.stringify(promptData.prompt).substring(0, 500),
            );
          }
          ...
          if (result) {
            return {
              ...
              meta: {
                ...
                promptVersion:
                  (promptData as any)?.version ??
                  (promptData as any)?.config?.version,
        ...
        console.warn(
          `⚠️  Failed to load prompt "${promptKey}" from Langfuse:`,
          error,
        );
    ...
    try {
      ...
      console.log(
        `📁 Loaded prompt "${promptKey}" from local fallback (${config.fallback_file})`,
      );
      ...
      throw new Error(
        `Invalid prompt format in ${config.fallback_file} - no content found`,
      );
    } catch (error) {
      throw new Error(
        `Failed to load prompt "${promptKey}" from both Langfuse and local fallback: ${error}`,
      );
    }
  }
  ...
  private parsePromptTemplate(
    ...
    if (content.includes("---") && content.includes("USER PROMPT:")) {
      ...
      if (parts.length >= 2) {
        ...
        const userPart = parts[1].replace(/^USER PROMPT:\s*/i, "").trim();
    ...
    const systemPrompt = content.replace(/^SYSTEM INSTRUCTION:\s*/i, "").trim();
  ...
  private compileTemplate(
    ...
    for (const [key, value] of Object.entries(variables)) {
      ...
      const singleBracePattern = new RegExp(
        `\\{${key}\\}(?![\\s]*[:\\[\\{])`,
        "g",
      );
   ...
   */
  async executeChatSQLGenerator(input: SQLGeneratorInput): Promise<{
    ...
    console.log(
      `   📝 Conversation history (${input.conversationHistory?.length || 0} messages):`,
    );
    ...
    console.log(
      `      - Compiled user prompt (first 200): ${userPrompt.substring(0, 200)}...`,
    );

    // Check if variable was substituted - if not, always prepend the conversation history
    const historyInSystem =
      systemPrompt.includes("Keine vorherige Konversation") ||
      systemPrompt.includes("Nutzer:");
    const historyInUser =
      userPrompt.includes("Keine vorherige Konversation") ||
      userPrompt.includes("Nutzer:");
    ...
    console.log(
      `   ✓ Prompts compiled (history in system: ${historyInSystem}, in user: ${historyInUser})`,
    );
    ...
    const langfusePrompt = await langfuseService.getPrompt(
      meta.promptName,
      meta.promptVersion && meta.promptVersion !== "fallback"
        ? Number(meta.promptVersion)
        : undefined,
    );
    ...
    try {
      ...
      const { data, usage } =
        await openRouterService.generateJSON<SQLGeneratorOutput>(userPrompt, {
          model: model,
      ...
      console.log(
        `     SQL: ${data.sql ? data.sql.substring(0, 100) + "..." : "<NONE>"}`,
      );
      console.log(`     Confidence: ${data.confidence}`);
      console.log(
        `     Needs clarification: ${data.needsClarification || "<NONE>"}`,
      );
   ...
   */
  async executeChatAnswerFormatter(input: AnswerFormatterInput): Promise<{
    ...
    console.log(
      `   Question: "${input.userQuestion?.substring(0, 80) ?? "<no question>"}..."`,
    );
    ...
    try {
      ...
      // Call OpenRouter with config from YAML
      const { data, usage } =
        await openRouterService.generateJSON<AnswerFormatterOutput>(
          userPrompt,
          {
            model: model,
            systemInstruction: systemPrompt,
   ...
   */
  async executeQuizQuestionGenerator(input: QuestionGeneratorInput): Promise<{
      ...
      schema: {
        ...
        properties: {
          questions: {
            ...
            items: {
              ...
              properties: {
                ...
                correct_answer: {
                  type: "string",
                  description:
                    "The correct answer extracted from the Knowledge Base data",
                },
                category: {
                  type: "string",
                  description:
                    "Category of the question (e.g., Tore, Spieler, Saison)",
                },
                difficulty: {
                  type: "string",
            ...
            description:
              "Array of quiz questions with correct answers from Knowledge Base",
          },
        },
        required: ["questions"],
        additionalProperties: false,
    ...
    console.log(
      `   📦 DEBUG - Normalized ${normalizedData.questions.length} questions`,
    );
   ...
   */
  async executeQuizAnswerGenerator(input: AnswerGeneratorInput): Promise<{
    ...
    console.log(
      `   Question: "${input.question?.substring(0, 80) ?? "<no question>"}..."`,
    );
    ...
    try {
        ...
        schema: {
          ...
          properties: {
            incorrectAnswers: {
              ...
              items: { type: "string" },
              description:
                "Array of 3 plausible but incorrect answer alternatives",
            },
            explanation: {
              type: "string",
              description:
      ...
      // Call OpenRouter with JSON Schema for structured output
      const { data: rawData, usage } =
        await openRouterService.generateJSON<any>(userPrompt, {
          model: model,
          systemInstruction: systemPrompt,
          temperature: config.llm_config.temperature,
          maxOutputTokens: config.llm_config.max_tokens,
      ...
      console.log(
        `     Wrong options: ${normalizedData.incorrectAnswers.map((a) => `"${a}"`).join(", ")}`,
      );
   ...
   */
  async executeKnowledgeBaseGenerator(input: KnowledgeBaseInput): Promise<{
    ...
    try {
      ...
      const germanDifficulty =
        difficultyMap[input.schwierigkeitsgrad] || input.schwierigkeitsgrad;
              ...
              items: {
                ...
                properties: {
                  sql_query: {
                    type: "string",
                    description:
                      "A valid PostgreSQL query for FSV Mainz 05 data",
                  },
                  reason: {
                    type: "string",
                    description:
                      "Explanation of what fact this query reveals and potential question hint",
                  },
                },
      ...
      const { data: rawData, usage } =
        await openRouterService.generateJSON<KnowledgeBaseOutput>(userPrompt, {
          model: model,
          systemInstruction: systemPrompt,
          temperature: config.llm_config.temperature,
          maxOutputTokens: config.llm_config.max_tokens,
